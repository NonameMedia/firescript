...
const LogicalExpression = require('../../../src/fs-nodes/LogicalExpression')
  ...
  describe('instance', () => {
    ASSIGNMENT_OPERASTORS.forEach((operator) => {
      it(`returns a LogicalExpression, using '${operator}'`, () => {
        ...
        const node = new LogicalExpression(tokenStack)
    ...
    ASSIGNMENT_OPERASTORS.forEach((operator) => {
      it(`returns a LogicalExpression, using '${operator}', get left from outside`, () => {
        const tokenStack = new TokenStack([
          { 'type': 'numeric', 'value': '6' },
          { 'type': 'operator', 'value': operator },
          { 'type': 'numeric', 'value': '7' }
        ...
        const node = new LogicalExpression(tokenStack, null, left)
